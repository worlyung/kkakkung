...
export async function listAlbumComments(albumId: string): Promise<Map<string, PhotoComment[]>> {
  ...
  const { data, error } = await supabase
    .from("comments")
    .select("id, body, created_at, photo_id, photos!inner(album_id), viewers(name)")
...
export async function addComment(albumId: string, photoId: string, viewerId: string, body: string): Promise<PhotoComment> {
  const supabase = createSupabaseServiceClient();
  ...
  const { data, error } = await supabase
    .from("comments")
    .insert({ photo_id: photoId, viewer_id: viewerId, body })
    .select("id, body, created_at, photo_id, viewers(name)")
    .single();
  ...
  const name = Array.isArray(viewer) ? viewer[0]?.name : viewer?.name;
  return { id: data.id, photoId: data.photo_id, name: name ?? "가족", body: data.body, createdAt: data.created_at };
}
